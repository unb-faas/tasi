...
const table='tb_search_question_answer as a'
const defaultFields = [
    'a.id',
    'a.id_search_question',
    'a.description',
 ]
...
const getPage = async (queryParams) => {
    ...
    /* Filtering */
    if(queryParams.filterSearchQuestion) {
        query = query.andWhereRaw(" a.id_search_question = ?", [queryParams.filterSearchQuestion])                        
    }
    if(queryParams.filterName) {
        query = query.andWhereRaw(' a.description like ?', [`%${queryParams.filterName}%`])                        
    }
